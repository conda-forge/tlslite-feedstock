About tlslite-feedstock
=======================

Feedstock license: [BSD-3-Clause](https://github.com/conda-forge/tlslite-feedstock/blob/main/LICENSE.txt)
...
Once the `conda-forge` channel has been enabled, `tlslite` can be installed with `conda`:
...
```
mamba install tlslite
```

It is possible to list all of the versions of `tlslite` available on your platform with `conda`:
...
```
mamba search tlslite --channel conda-forge
```
...
mamba repoquery search tlslite --channel conda-forge

# List packages depending on `tlslite`:
mamba repoquery whoneeds tlslite --channel conda-forge

# List dependencies of `tlslite`:
mamba repoquery depends tlslite --channel conda-forge
```
